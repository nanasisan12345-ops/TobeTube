...
```bash
npm run sync:youtube -- --limit=90 --target-count=50 --discovery-limit=5 --discovery-target-count=12
```
...
1回の実行では通常検索を最大90組、発掘検索を最大5組、合計95組に抑えます。GitHub Actionsは毎日1回実行され、候補が少ない組み合わせから自動的に追加してコミットします。手動実行では `country` と `genre` を指定して、たとえば `jp`×`game`だけを優先的に増やすこともできます。現在の0本の組み合わせは `npm run coverage` で確認できます。
