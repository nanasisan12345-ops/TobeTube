...
- 通常モードは、選んだ国とジャンルの中で、再生数が多い上位25%の人気動画を優先します。
- 発掘モードは、選んだ国とジャンルの中で、再生数が少ない下位25%の動画を最優先します。視聴履歴と直前ジャンルは、その低再生数候補内で重複を避けるためだけに使います。再生数が未取得の候補だけの場合は、視聴履歴による重複回避へ切り替わります。
...
```bash
npm run sync:youtube -- --limit=90 --target-count=50
```
...
同期時には、目標の50本より動画が少ない国×ジャンルを本数の少ない順で検索し、1検索につき条件を満たす最大50本を追加します。既存動画を含めて再生数も更新します。発掘モードはこの値を使い、選択範囲内で再生数が少ない上位25%からランダムに選びます。同じ一本だけに偏らず、埋もれた動画を優先するための方式です。

1回の実行では検索を最大90組に抑えます。GitHub Actionsは毎日1回実行され、候補が少ない組み合わせから自動的に追加してコミットします。手動実行では `country` と `genre` を指定して、たとえば `jp`×`game`だけを優先的に増やすこともできます。現在の0本の組み合わせは `npm run coverage` で確認できます。
