...
ハートで動画を保存し、ヘッダーのコレクションを開くと、6軸のレーダーチャート・割合・ジャンル構成を表示します。「お気に入り」「最近見た動画」タブは従来どおり利用できます。追加・削除後に再計算し、保存済みのお気に入りもそのまま対象になります。

- 知識: 科学、学び、歴史、ドキュメンタリー、テクノロジー
- 創作: ものづくり、アート、音楽、ダンス
- 刺激: ゲーム、ホラー、スポーツ
- 物語: 映画、アニメーション、コメディ
- 癒やし: 動物、ライブカメラ、リラックス、ASMR
- 暮らし・旅: 料理、旅、乗りもの

各動画を1分類で同じ重みにし、分析可能な動画を分母にした百分率を四捨五入して表示します（合計が100%にならない場合があります）。子ジャンルは親分類を継承。最多の軸をタイプ名にし、同率はミックスタイプ、5本未満は暫定表示です。取得できないIDや未知の分類は対象外の本数を表示します。新しい大分類を追加するときは `js/taste-profile.js` の分類とテストも更新してください。

全ての国のお気に入りが対象です。視聴履歴・再生数・動画タイトルによる推測や外部AIは使いません。実際の性格・能力を診断する機能ではありません。分析処理はブラウザー内だけで行い、分析結果の外部送信や追加保存はありません。既存のお気に入り保存を削除すればプロフィールもリセットされます。国選択に連動した12言語とキーボードの左右矢印/Home/Endによるタブ操作に対応します。
...
- 通常モードは、発掘専用候補を除いた国とジャンルの中で、再生数が多い上位25%の人気動画を優先します。短尺と通常尺の両方がある場合、短尺の抽選率は約15%に抑えます。
- 発掘モードは、YouTube Data APIで新着順に別収集した候補を再生数の少ない順に選別し、その専用候補を最優先します。専用候補がまだない国・ジャンルでは、既存候補の再生数下位25%へ切り替わります。視聴履歴と直前ジャンルは、低再生数候補内で重複を避けるためだけに使います。発掘モードも短尺の抽選率は約15%です。
...
- YouTube上で公開中
- 埋め込み再生が許可されている
- キーワード検索ではセーフサーチを厳格に指定した検索結果
- 既存カタログと動画IDが重複しない
- 国×ジャンルごとの短尺比率が20%を超えない候補（通常尺が存在しない検索結果では追加を見送ります）

通常候補のうちYouTube公式カテゴリと明確に対応できる11ジャンルは、最初に国別の人気チャートから取得します。この取得方法はキーワード検索枠を消費せず、1回で最大50本を取得できます。同じ動画が複数の国の人気チャートに現れた場合は、動画を重複登録せず対象国だけを追加します。

国別人気チャートの次に、登録済み動画のチャンネル投稿一覧を低コストAPIで巡回します。動画の言語またはチャンネル所在国が選択国と一致し、YouTube公式カテゴリも対象ジャンルと一致する公開・埋め込み可能な動画だけを採用します。1日50チャンネル、1チャンネル最大100投稿を確認し、公式カテゴリ対応ジャンルは国ごとに100本を目標とします。処理済みの国×ジャンル×チャンネルは `data/youtube-channel-sync.json` へ記録し、同じ一覧を毎日繰り返しません。
